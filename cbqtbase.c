...
int CB_StrPrint(int argc, CBQArg_t* argv)
{
    if (argc == 1)
        printf("CB: %s\n", argv[0].sVar);
    return 0;
...
{
    char *strings[] = {
        "This is from first queue",
        "This is from second queue"
    };

    CBQueue_t q1, q2;
    CBQ_QueueInit(&q1, CBQ_SI_TINY, CBQ_SM_LIMIT, CBQ_SI_SMALL, 0);
    CBQ_QueueInit(&q2, CBQ_SI_TINY, CBQ_SM_STATIC, 0, 0);

    for (int i = 0; i < CBQ_SI_TINY; i++)
        CBQ_PushN(&q1, CB_StrPrint, {.sVar = strings[0]}),
        CBQ_PushN(&q2, CB_StrPrint, {.sVar = strings[1]});

    CBQ_QueueTransfer(&q1, &q2, CBQ_SI_TINY, 1, 1);

    for (int i = 0; i < CBQ_SI_TINY * 2; i++)
        CBQ_Exec(&q1, 0);
...
int CB_PrintNum(int argc, CBQArg_t* argv)
{
    if (argc == 1)
        printf("CB: %d\n", argv[0].iVar);
...
{
    CBQueue_t queue;
    CBQ_QueueInit(&queue, CBQ_SI_TINY, CBQ_SM_LIMIT, CBQ_SI_TINY * 2, 0);
